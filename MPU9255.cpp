...
void MPU9255::setBandWidth(const uint16_t hzFreq)
{
	uint8_t dlpf;
	if (hzFreq > 184)
		dlpf = 0; // 460Hz Acc, FS = 1KHz // 250Hz Gyro, 4000Hz Temp, FS = 8KHz
	else if (hzFreq > 92)
		dlpf = 1; // 184Hz Gyro/Acc, 4000Hz Temp, FS = 1KHz
	else if (hzFreq > 41)
		dlpf = 2; // 92Hz Gyro/Acc, 4000Hz Temp, FS = 1KHz
	else if (hzFreq > 20)
		dlpf = 3; // 41Hz Gyro/Acc, 42Hz Temp, FS = 1KHz
	else if (hzFreq > 10)
		dlpf = 4; // 20Hz Gyro/Acc, 20Hz Temp, FS = 1KHz
	else if (hzFreq > 5)
		dlpf = 5; // 10Hz Gyro/Acc, 10Hz Temp, FS = 1KHz
	else
		dlpf = 6; // 5Hz Gyro/Acc, 5Hz Temp, FS = 1KHz
...
void MPU9255::calibAccelGyro(void)
{
	...
	{
		getMeasurement();
		sumAcX += xyz_AccTmpGyr[0];
		sumAcY += xyz_AccTmpGyr[1];
		sumAcZ += xyz_AccTmpGyr[2];
		sumGyX += xyz_AccTmpGyr[4];
		sumGyY += xyz_AccTmpGyr[5];
		sumGyZ += xyz_AccTmpGyr[6];
...
void MPU9255::calcAccelYPR(float &raw_x, float &raw_y, float &raw_z, float &angle_x, float &angle_y, float &angle_z)
{
	...
	angle_z = accel_angle_z;
}

void MPU9255::calcGyroYPR(float &raw_x, float &raw_y, float &raw_z, float &angle_x, float &angle_y, float &angle_z)
{
...
{ // { x, y }
	...
	float acc_angle_x, acc_angle_y, acc_angle_z, gyr_angle_x, gyr_angle_y, gyr_angle_z;

	calcAccelYPR(acc_x, acc_y, acc_z, acc_angle_x, acc_angle_y, acc_angle_z);
	calcGyroYPR(gyr_x, gyr_y, gyr_z, gyr_angle_x, gyr_angle_y, gyr_angle_z);
	calcFilteredYPR();
...
void MPU9255::getResult(float *acc_buffer,
						...
						size_t filter_angles_len)
{
	...
	if ((acc_buffer != nullptr) && (acc_buffer_len >= 6))
	{
		acc_buffer[0] = acc_x;
		acc_buffer[1] = acc_y;
		acc_buffer[2] = acc_z;
		acc_buffer[3] = acc_angle_x;
		acc_buffer[4] = acc_angle_y;
		acc_buffer[5] = acc_angle_z;
	}
	else
	{
		printf("Invalid accelerometer argument");
	}

	if ((gry_buffer != nullptr) && (gyr_buffer_len >= 6))
	{
		gry_buffer[0] = gyr_x;
		gry_buffer[1] = gyr_y;
		gry_buffer[2] = gyr_z;
		gry_buffer[3] = gyr_angle_x;
		gry_buffer[4] = gyr_angle_y;
		gry_buffer[5] = gyr_angle_z;
	}
	else
	{
		printf("Invalid gyroscope argument");
	}

	if ((filter_angles != nullptr) && (filter_angles_len >= 3))
	{
		filter_angles[0] = filtered_angle_x;
		filter_angles[1] = filtered_angle_y;
		filter_angles[2] = 0;
	}
	else
	{
